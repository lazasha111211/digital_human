...
class IndexTTS2:
    def __init__(
        ...
        current_work_dir = os.getcwd()  # 获取当前工作目录（运行Python时的目录）
        
      
        extract_features = os.path.join(current_work_dir, "checkpoints/facebook_w2v-bert-2.0")
        print(f"facebook path is {extract_features}")
        self.extract_features = SeamlessM4TFeatureExtractor.from_pretrained(extract_features)
        ...
        semantic_code_ckpt = os.path.join(current_work_dir, "checkpoints/amphion_MaskGCT/semantic_codec/model.safetensors")
        ...
        campplus_ckpt_path = os.path.join(current_work_dir, "checkpoints/funasr_campplus/campplus_cn_common.bin")
        ...
        bigvgan_name = os.path.join(current_work_dir, "checkpoints/nvidia_bigvgan") # self.cfg.vocoder.name
